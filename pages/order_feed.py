...
import allure
from utils.locators import FeedPageLocators, MainPageLocators
...
class OrderFeedPage(BasePage):
    ...
    def wait_for_orders_line_header_loaded(self):
        self.wait_loading_of_element(FeedPageLocators.FEED_HEADER)

    @allure.step('Получаем ID первого заказа')
    def get_first_order_id(self):
        return self.driver.find_element(By.XPATH, FeedPageLocators.FIRST_ORDER[1]).text
    ...
    def check_order_details_modal_opened(self):
        try:
            self.driver.find_element(By.XPATH, FeedPageLocators.ORDER_DETAILS_MODAL)
    ...
    def get_order_id_from_modal(self):
        return self.driver.find_element(By.XPATH, FeedPageLocators.ORDER_DETAILS_MODAL_ORDER_ID_XPATH).text
    ...
    def get_total_count(self):
        return self.driver.find_element(By.XPATH, FeedPageLocators.TOTAL_COUNT_XPATH).text

    @allure.step('Получаем значение счетчика Выполнено за сегодня')
    def get_today_count(self):
        return self.driver.find_element(By.XPATH, FeedPageLocators.TODAY_COUNT_XPATH).text
